...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== GET ALL ELEMENTS =====
    const girlfriendVideo = document.getElementById('girlfriendVideo');
    const playButton = document.getElementById('playButton'); // Make sure this ID exists!
    const vinylDisc = document.getElementById('vinylDisc');
    
    // Check if elements exist
    if (!girlfriendVideo) {
        console.error('❌ Video element not found! Check ID "girlfriendVideo"');
        return;
    }
    
    if (!playButton) {
        console.error('❌ Play button not found! Check ID "playButton"');
        return;
    }
    
    if (!vinylDisc) {
        console.error('❌ Vinyl disc not found! Check ID "vinylDisc"');
        return;
    }
    ...
    playButton.addEventListener('click', function() {
        if (!isPlaying) {
            ...
            // Play the video
            girlfriendVideo.play()
                .then(() => {
                    console.log('🎵 Playing girlfriend singing');
                    isPlaying = true;
                })
                .catch(error => {
            ...
            girlfriendVideo.pause();
            girlfriendVideo.currentTime = 0;
    ...
    // ===== VIDEO ENDED =====
    girlfriendVideo.addEventListener('ended', function() {
        vinylDisc.classList.remove('playing');
        playButton.classList.remove('playing');
        playButton.innerHTML = '<i class="fas fa-play"></i>';
        isPlaying = false;
    ...
    // ===== ERROR HANDLING =====
    girlfriendVideo.addEventListener('error', function(e) {
